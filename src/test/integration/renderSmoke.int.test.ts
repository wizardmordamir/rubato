...
describe('rubato-render-smoke CLI (injected runner — no browser/server)', () => {
  ...
  it('exits 1 and prints RED + error detail on a white screen', async () => {
    ...
    const code = await run(['--port', '4902'], io, async () =>
      result({
        ran: true,
        ok: false,
        detail: 'WHITE SCREEN — React root (#root) is empty after load',
        probe: {
          launched: true,
          navigated: true,
          rootFound: true,
          rootHtmlLength: 0,
          consoleErrors: ['Hook dispatcher is null'],
          pageErrors: [],
        },
      }),
    );
  ...
  it('exits 0 (lenient) when INCONCLUSIVE — no browser available', async () => {
    const io = captureIo();
    const code = await run(['--port', '4903'], io, async () =>
      result({ ran: false, ok: false, detail: 'render check could not run: playwright not installed' }),
    );
    expect(code).toBe(0);
  ...
  it('exits 2 under --strict when INCONCLUSIVE', async () => {
    const io = captureIo();
    const code = await run(['--port', '4904', '--strict'], io, async () =>
      result({ ran: false, ok: false, detail: 'no chrome' }),
    );
    expect(code).toBe(2);
  });
  ...
  it('threads --cwd / --url / --root / --nav-timeout into the spec', async () => {
    ...
    const io = captureIo();
    await run(
      ['--port', '4905', '--cwd', '/some/wt', '--url', '/apps', '--root', '#app', '--nav-timeout', '7000'],
      io,
      async (spec) => {
        seen = {
          cwd: spec.cwd,
